...
module.exports = (err, req, res, next) => {
  //console.log(err.stack);  //err.stack gives us the stack trace
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    //let error = { ...err }; //It didn't worked as it gives undefined
    let error = Object.create(err);
    const errName = error.name;
    //console.log(errName);

    //if (error.message.includes('Cast')) error = handleCastErrorDB(error);
    if (errName === 'CastError') error = handleCastErrorDB(error);
    if (error.code === 11000) error = handleDuplicateFieldsDB(error);
    if (errName === 'ValidationError') error = handleValidationErrorDB(error);
